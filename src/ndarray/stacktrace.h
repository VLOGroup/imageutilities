...
/*
int inline fprintf(std::ostream & out, const char * format, ...){
	std::size_t found = std::string(format).find('%');
	std::cout << format;
	if (found == std::string::npos){// no format entries, treat as text only
		out << format <<"\n";
		return 0;
	}else{
		char * s[1024*16];
		...
		int r = vsprintf((char*)s,format, argp);
		va_end(argp);
		out << s;
		std::cout << s;
		return r;

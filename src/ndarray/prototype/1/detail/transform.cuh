...
*/
	...
	//---------- functor parsers ---------------------------------------
	template<typename F, typename... AA> std::tuple<AA...> functor_args(const F & f, void (F::*method)(AA... )const){
		return std::tuple<AA...>();
	}

	template<typename F, int dims> struct functor_takes_index{
		...
		static constexpr bool value = std::is_same<f_first, tindex >::value;
	...
	*/
	...
	//template<typename T0, typename T1, void (*f)(T0 &, const T1&)> functor<T0,T1,f> make_functor(void (*g)(T0 &, const T1&)){
	//		return functor<T0,T1, f>();
	//	}
	...
	void transform(ndarray_ref<T0,dims> & a, const ndarray_ref<T0,dims>& a1){
		typedef void (*F)(T0 &, const T0&);
		//make_transform(policy_flatten(), a, a1).apply(f);
		//auto ff = make_functor<T0,T0 ,f>(f);
		auto ff = functor<decltype(f), f>();
		make_transform(policy_flatten(), a, a1).apply(ff);
	};
/*
	template<int dims, typename T0, typename... TT, void (*f)(T0 &, const TT&... )>
		...
		make_transform(policy_flatten(), a, aa...).apply(f);
	}
*/
	/*
	template<int dims, typename T0, typename... TT>
	__attribute__((noinline))
	...
	void transform(const F & f, A0 & a, const AA&... aa){
		...
		constexpr bool indexed = functor_takes_index<F,dims1>::value;
		typedef typename std::conditional<indexed, policy_indexed, policy_flatten>::type policy;
		make_transform(policy(), a, aa...).apply(f);
	}

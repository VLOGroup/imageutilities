...
inline static __device__ PixelType dxp(cudaTextureObject_t tex, const int x, const int y,
	const int width, const int height)
{
	float val_x = tex2D<float>(tex, x + 1.5f, y + 0.5f) - tex2D<float>(tex, x + 0.5f, y + 0.5f);
...
template<typename PixelType>
inline static __device__ PixelType dyp(cudaTextureObject_t tex, const int x, const int y,
	const int width, const int height)
{
	PixelType val_y = tex2D<PixelType>(tex, x + 0.5f, y + 1.5f) - tex2D<PixelType>(tex, x + 0.5f, y + 0.5f);
...
template<typename PixelType>
inline static __device__ float2 dp(cudaTextureObject_t tex,
	const int x, const int y)
{
	float2 grad = make_float2(0.0f, 0.0f);
	PixelType cval = tex2D<PixelType>(tex, x + 0.5f, y + 0.5f);
	grad.x = tex2D<PixelType>(tex, x + 1.5f, y + 0.5f) - cval;
	grad.y = tex2D<PixelType>(tex, x + 0.5f, y + 1.5f) - cval;
	return grad;
}

template<typename PixelType>
inline static __device__ float2 dp_c(cudaTextureObject_t tex,
	const int x, const int y)
{
	float2 grad = make_float2(0.0f, 0.0f);
	PixelType cval_x = tex2D<PixelType>(tex, x - 0.5f, y + 0.5f);
	PixelType cval_y = tex2D<PixelType>(tex, x + 0.5f, y - 0.5f);
	grad.x = tex2D<PixelType>(tex, x + 1.5f, y + 0.5f) - cval_x;
	grad.y = tex2D<PixelType>(tex, x + 0.5f, y + 1.5f) - cval_y;
	return grad / 2;
}

template<typename PixelType>
__global__ void KMagnitude(cudaTextureObject_t texX, cudaTextureObject_t texY, float *outMagnitude, int width, int height, int stride)
{
	...
	float dx = (float)tex2D<PixelType>(texX, x + 0.5, y + 0.5);
	float dy = (float)tex2D<PixelType>(texY, x + 0.5, y + 0.5);

	outMagnitude[y * stride + x] = sqrt(dx*dx + dy*dy);
}

...
extern void cuReduce(iu::ImageGpu_32f_C4* src, iu::ImageGpu_32f_C4* dst,
                         IuInterpolationType interpolation, cudaStream_t stream=0);
...
//    unsigned int kernel_size = 5;
    ...
    iuprivate::filterGauss(src, filtered, sigma, kernel_size);
...
//    unsigned int kernel_size = 5;
    ...
    //std::cout << "factor=" << factor << " sigma=" << sigma <<  " kernel_sz=" << kernel_size << std::endl;
    iuprivate::filterGauss(src, temp, sigma, kernel_size, temp_filter, stream);
                ...
                bool gauss_prefilter)
{
  ...
  if(gauss_prefilter)
  {
    filtered = new iu::ImageGpu_32f_C4(src->size());
    filtered_auto.reset( filtered );
